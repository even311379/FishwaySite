...
class Menu(ClusterableModel, Orderable):
    ...
    panels = [MultiFieldPanel([FieldPanel("title")], heading="Menu"), InlinePanel("menu_items", label="Menu Item")]
...
class GenericPageContent(BaseGenericSetting):
    banner_bg_image = models.ForeignKey(
        "wagtailimages.Image", on_delete=models.CASCADE, related_name="+", null=True, help_text="頁首底圖", blank=True
    )
