...
app.layout = html.Div(
    [
        ...
        dcc.Loading(
            ...
            type="graph",
            children=html.Div(
                "", id="table-content", className="w-[60vw] md:w-[40vw] mx-auto shadow-2xl rounded-sm bg-gray-100"
            ),
        ),
        html.Div(
            html.Button(
                "下載",
                id="download-button",
                className="my-6 bg-blue-500 hover:bg-blue-700 text-white font-bold px-12 rounded",
            ),
...
def update_table(camera_name, fish_name, start_date, end_date):
    query_set = PassCount.objects.filter(
        fish=fish_name, analysis__camera=camera_name, analysis__event_date__range=[start_date, end_date]
    ).values_list("analysis__event_date", "count")
    df = pd.DataFrame(list(query_set), columns=["event_date", "count"])
    df.columns = ["日期", "數量"]
...
def download_table(n_clicks, camera_name, fish_name, start_date, end_date):
    ...
    return dcc.send_data_frame(
        df.to_csv, f"碧潭堰魚道通過數量_{camera_name}_{fish_name}_{start_date}-{end_date}.csv", index=False
    )
